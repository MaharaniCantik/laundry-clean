<!-- Mulai Navigasi -->
  <nav x-data="{ open: false }" class="sticky top-0 w-full z-50 pt-4 px-4">
    <div class="max-w-7xl mx-auto bg-white/90 backdrop-blur-md border border-gray-100 shadow-lg rounded-[30px] px-6 lg:px-10">
      <div class="flex justify-between items-center h-20">
        ...
        <div class="hidden md:flex items-center space-x-6 lg:space-x-8">
            <x-nav-link href="{{ route('welcome') }}" class="text-gray-700 hover:text-[#F6921E]">
                Beranda
            </x-nav-link>

            <x-nav-link href="{{ url('/#tentang-kami') }}" class="text-gray-700 hover:text-[#F6921E]">
                Tentang Kami
            </x-nav-link>

            <x-nav-link href="{{ url('/#layanan') }}" class="text-gray-700 hover:text-[#F6921E]">
                Layanan
            </x-nav-link>

            <x-nav-link href="{{ route('lacak') }}" class="text-gray-700 hover:text-[#F6921E]">
                Lacak
            </x-nav-link>

            <x-nav-link href="{{ url('/#kontak') }}" class="text-gray-700 hover:text-[#F6921E]">
                Kontak
            </x-nav-link>
        </div>
          ...
          @auth
            <div class="flex items-center space-x-4">
              ...
              <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="bg-red-500 text-white px-4 py-2 rounded-full text-xs font-bold hover:bg-red-600 transition">
                  Keluar
                </button>
              </form>
            </div>
